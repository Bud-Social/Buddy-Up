...
patched label map and class-balanced train/test manifests under
``data/processed/food``, and validates that no class is missing.

Since buddy_data.food101()/food101_splits() load Food-101 from the HF Hub
(``ethz/food101``, train/validation/test), this script materialises the split
images it references to ``data/processed/food/images/<split>/<class>/`` (the
canonical Food-101 layout the notebooks expect) and points the manifests at
those files. First run downloads ~5 GB via `datasets`.
...
-------
data/processed/food/
├── food101_label_map.csv     # class, nutrition_key, bucket (+ macros)
├── images/<split>/<class>/*.jpg
...
from buddy_data import food101_splits
...
def _materialise_split(ds, images_root: Path, out: Path) -> int:
    """Write split images to disk + manifest rows (image_path, class, ...)."""
    ...
    with open(out, 'w', newline='') as fh:
        ...
        for idx, ex in enumerate(ds):
            ...
            img = images_root / cls / f'{idx:06d}.jpg'
            if not img.exists():
                img.parent.mkdir(parents=True, exist_ok=True)
                ex['image'].convert('RGB').save(img, 'JPEG')
            key, bucket = FOOD101_LABELS[cls]
            writer.writerow({
                'image_path': str(img),
                'class': cls,
...
def main():
    ...
    n_train = _materialise_split(splits['train'], PROCESSED / 'images' / 'train',
                                 PROCESSED / 'train.csv')
    n_test = _materialise_split(splits['test'], PROCESSED / 'images' / 'test',
                                PROCESSED / 'test.csv')
