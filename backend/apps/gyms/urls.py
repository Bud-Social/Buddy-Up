...
urlpatterns = [
    ...
    path('<str:gym_slug>/join-requests/<uuid:request_id>/', views.ManageJoinRequestView.as_view(), name='manage_join_request'),
    path('<str:gym_slug>/invite/', views.InviteCreateView.as_view(), name='invite'),
    path('<str:gym_slug>/invites/<uuid:invite_id>/<str:action>/', views.InviteActionView.as_view(), name='invite_action'),
