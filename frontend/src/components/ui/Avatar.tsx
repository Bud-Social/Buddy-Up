...
export function Avatar({ src, alt, size = 'md', showRepRing = false, streakProgress = 0, className, onClick, style }: AvatarProps) {
  const img = (
    <div className={`rounded-full bg-buddy-surface overflow-hidden flex-shrink-0 ${sz[size]} ${className || ''}`} onClick={onClick} role={onClick ? 'button' : undefined} tabIndex={onClick ? 0 : undefined} style={style}>
